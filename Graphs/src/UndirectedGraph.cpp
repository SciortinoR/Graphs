...
bool UndirectedGraph::addVertices(vector<Vertex> v)
{
	unsigned int size = vSet.size();
...
bool UndirectedGraph::removeEdges(vector<Edge> e)
{
	unsigned int size = eSet.size();
...
void UndirectedGraph::display(const Vertex& v)
{
	...
	do
	{
		...
		if (size == AdjacentList.size())	//if node not removed from stack
		{
			...
			for (unsigned int i=0; i<eSet.size(); i++)
				...
				else if (topID == eSet[i].getEnd().getID())
				{
					int flag = 0;
					for (unsigned int j=0; j<visited.size(); j++)		//checks if adjacent node already visited
						if (eSet[i].getStart().getID() == visited[j])
						{
							flag++;
							break;
						}
					if (flag == 0)
					{
						AdjacentList.push(eSet[i].getStart());		//adds adjacent node to stack
						pushCounter++;
					}
				}
...
void UndirectedGraph::display(Edge& e)
{
	...
	do
	{
		...
		{
			//Initializing path with edge starting vertex
			path = to_string(e.getStart().getID());
			path.push_back('-');
			path.push_back('>');
		}
		else
		{
			//Initializing path with edge end vertex
			path = to_string(e.getEnd().getID());
			path.push_back('-');
			path.push_back('>');

			visited.push_back(e.getEnd().getID());		//stores initial end as visited
			AdjacentList.push(e.getStart());			//pushes back start vertex of edge in stack to start path output
			index++;
		}
...
void UndirectedGraph::display() const
{
	...
	for (unsigned int i=0; i<eSet.size(); i++)
	{
		...
		cout<<"Connecting Vertex 1 ID - Value: "<<eSet[i].getStart().getID()<<" - "<<eSet[i].getStart().getValue()<<endl;
		cout<<"Connecting Vertex 2 ID - Value: "<<eSet[i].getEnd().getID()<<" - "<<eSet[i].getEnd().getValue()<<endl<<endl;
	}
}

//converts graph to string
void UndirectedGraph::toString()
{
