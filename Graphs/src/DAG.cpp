...
bool DAG::addVertices(vector<Vertex> v)
{
	unsigned int size = vSet.size();
...
bool DAG::addEdge(Edge& e)
{
	...
	for (unsigned int i=0; i<vSet.size(); i++)			//checks if starting vertex in graph
		if (e.getStart() == vSet[i])
		{
			for (unsigned int j=0; j<vSet.size(); j++)	//checks if ending vertex is in graph
				if (e.getEnd() == vSet[j])
				{
					Adjacent.push_back(e.getEnd());

					do			//adds edge if doesn't loop back
					{
						for (unsigned int p=0; p<Adjacent.size(); p++)			//checks if adjacent vertices equivalent to edge start vertex
							if (Adjacent[p] == e.getStart())
							{
								cout<<"Edge Not Added (Creates Loop)!"<<endl;
								return false;
							}

						for (unsigned int l=0; l<eSet.size(); l++)			//adds adjacent vertices
							if (Adjacent[0] == eSet[l].getStart())
								Adjacent.push_back(eSet[l].getEnd());
						Adjacent.erase(Adjacent.begin());										//pops last vertex out
					}
...
bool DAG::removeEdges(vector<Edge> e)
{
...
bool DAG::searchVertex(const Vertex& v)
{
	for (unsigned int i=0; i<vSet.size(); i++)
		if (vSet[i] == v)
		{
...
bool DAG::searchEdge(const Edge& e)
{
	for (unsigned int i=0; i<eSet.size(); i++)
		if (eSet[i] == e)
		{
...
void DAG::display(const Vertex& v)
{
...
void DAG::display(Edge& e)
{
	vector<int> visited;			//visited node vector
	stack<Vertex> AdjacentList;		//Adjacent node stack

	//Initializing path with starting vertex
